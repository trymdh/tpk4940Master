...
def imageConfig(hDev):
    ...
    cam.cx_setParam(hDev,"ExposureTimeAbs", 5000)# default 1500
    cam.cx_setParam(hDev,"ReverseY", 1)
...
def checkStatusCode(code):
    #Outputs the status code given in the error from the AT camera in text format.
...
def pixCoordify(image,width):
    ...
    """
    new_array = list(range(width))
    new_array = np.vstack((new_array,image)).T
    return new_array